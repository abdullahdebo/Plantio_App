...
class _HomePageState extends State<HomePage> {
        ...
        await predictImage(base64Image).then((prediction) async {
          if (prediction != null) {
            if (prediction['result'] != null &&
                prediction['result']['classification'] != null &&
                prediction['result']['classification']['suggestions'] != null) {
              Map<String, dynamic> plantSuggestions =
                  prediction['result']['classification']['suggestions'][0];
              // Apply Plant Search by Name
              await getImageSearch(plantSuggestions['name'])
                  .then((plantSearch) async {
                if (plantSearch != null) {
                  if (plantSearch['entities'] != null) {
                    await getImageSearchDetails(
                            plantSearch['entities'][0]['access_token'])
                        .then((predictedPlantDetails) async {
                              ...
                              builder: (context) {
                                return Container(
                                  height: MediaQuery.of(context).size.height *
                                      0.85, // 85% of the screen height
                                  child: PlantDetails(
                                      predictedImage: predictedPlantDetails),
                                );
                              },
  ...
  Future<Map<String, dynamic>?> predictImage(String imageUrlB64) async {
    ...
    var data = {
      'images': ['data:image/png;base64,$imageUrlB64'],
      'similar_images': true
    };
    var dio = Dio();
  ...
  Future<Map<String, dynamic>?> getImageSearchDetails(
      String plantAccessToken) async {
    var headers = {
      'Api-Key': '4LdLuJXhFGTSRTgJgkKJj3WfUh0QO9EyqtIhb1R2Qjr1bUa1mX'
    };
    var dio = Dio();
